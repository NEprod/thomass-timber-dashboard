...
from components.customer_details import CustomerDetailsBlock
from components.job_totals import JobTotalsBlock
...
class JobSheetWindow(tk.Toplevel):
    ...
    def update_visible_fields(self):
        ...
        if hasattr(self, "square_inputs"):
            self.square_inputs.update_layout_for_job_type(panel_type)  # handle special Dado layout
        if hasattr(self, "dado_inputs"):
            self.dado_inputs.update_layout_for_job_type(panel_type)  # handle special Dado layout

        # Add further control for mouldings and ledge later...
        self.check_for_bead_ledge()
        self.check_for_half_square_wall()
        self.update_ledge_options()
        self.update_material_totals_visibility()


    def on_panelling_change(self):
        """Triggered when the panelling type dropdown changes."""
        panel_type = self.panelling_type_dropdown.get_value()
        print(f"[DEBUG] Panel Type selected: {panel_type}")

        self.update_visible_fields()  # your existing UI logic

    def on_job_type_change(self, *args):
        job_type = self.job_type_dropdown.get_value()
        print(f"[DEBUG] Job Type selected: {job_type}")
    ...
    def update_job_totals(self):
        ...
        try:
            full_days = float(self.customer_details.vars["full_days_var"].get())
        except:
            full_days = 0

        try:
            extra_hours = float(self.customer_details.vars["extra_hours_var"].get())
        except:
            extra_hours = 0
    ...
    def check_for_half_square_wall(self, *arg):
        ...
        selected_panelling = self.job_type_dropdown.get_value().lower()


        for section in self.measurements:
            if "half wall" in panel_type:
    ...
    def check_for_bead_ledge(self, *arg):
        ...
        """
        selected_panelling = self.panelling_type_dropdown.get_value().lower()
        self.clear_measurements()
    ...
    def update_ledge_options(self):
        ...
        panelling = self.panelling_type_dropdown.get_value().lower()
        options = []
    ...
    def update_section_measurements(self, section):
        ...
        try:
            ...
            section['job_label'] = self.panelling_type_dropdown.get_value()
            ...
            if "half wall" in selected_panelling:
    ...
    def build_window(self):
        ...
        self.customer_details.frame.pack(fill="x", pady=(5, 10))
        ...
        self.job_type_dropdown.frame.pack(fill="x", pady=(5, 10))

        self.panelling_type_dropdown = PanellingTypeDropdownBlock(top_frame, on_change=self.on_panelling_change)
        self.panelling_type_dropdown.frame.pack(fill="x", pady=(5, 10))

        self.square_inputs = SquareInputBlock(
            parent=top_frame,
            material_data=self.material_data,
            on_change=self.on_panelling_change
        )
        self.square_inputs.show()
        ...
        self.job_totals.frame.pack(fill="x", pady=(5, 10))
