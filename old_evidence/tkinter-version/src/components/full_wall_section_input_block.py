...
class FullWallSectionInputBlock:
    def __init__(self, parent, index=1, on_change=None):
        ...
        self.vars['wall_height_var'] = tk.StringVar()
        ...
        ttk.Label(row, text="Wall Height(mm)").grid(row=0, column=2, sticky="w", padx=5, pady=5)
        ttk.Entry(row, textvariable=self.vars['wall_height_var']).grid(row=0, column=3, sticky="ew", padx=5, pady=5)
        ...
        # --- Bead Per Strip Measurements ---
        self.vars["bead_pieces_label"] = ttk.Label(row, text="Bead Strips Needed")
        self.vars["bead_pieces_label"].grid(row=5, column=0, sticky="w", padx=5, pady=5)
        self.vars["bead_pieces_entry"] = ttk.Entry(row, textvariable=self.vars['bead_strips_var'], state="readonly")
        self.vars["bead_pieces_entry"].grid(row=5, column=1, sticky="ew", padx=5, pady=5)
        ...
        def trigger_on_change(*_):
            print("[DEBUG]Full Wall Sections Changed")
            if self.on_change:
                self.on_change()
        ...
        self.vars["wall_height_var"].trace_add("write", trigger_on_change)
