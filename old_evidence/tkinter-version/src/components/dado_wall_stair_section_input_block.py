...
class DadoWallStairSectionInputBlock:
    def __init__(self, parent, index=1, on_change=None):
        ...
        self.vars['stair_tread_rise_var'] = tk.StringVar()
        self.vars['stair_tread_depth_var'] = tk.StringVar()
        self.vars['stair_step_count_var'] = tk.StringVar()
        ...
        ttk.Label(row, text="Stair Tread Rise(mm)").grid(row=2, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(row, textvariable=self.vars['stair_tread_rise_var']).grid(row=2, column=1, sticky="ew", padx=5, pady=5)
        ttk.Label(row, text="Stair Tread Depth(mm)").grid(row=2, column=2, sticky="w", padx=5, pady=5)
        ttk.Entry(row, textvariable=self.vars['stair_tread_depth_var']).grid(row=2, column=3, sticky="ew", padx=5, pady=5)
        ttk.Label(row, text="Step Count").grid(row=3, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(row, textvariable=self.vars['stair_step_count_var']).grid(row=3, column=1, sticky="ew", padx=5, pady=5)
        ...
        self.vars["inner_bottom_square_width_entry"].grid(row=7, column=1, sticky="ew", padx=5, pady=5)

        self.vars["inner_bottom_square_height_label"] = ttk.Label(row, text="Inner Bottom SqH(mm)")
        self.vars["inner_bottom_square_height_label"].grid(row=7, column=2, sticky="w", padx=5, pady=5)
        ...
        self.vars["inner_bottom_square_height_entry"].grid(row=7, column=3, sticky="ew", padx=5, pady=5)

        self.vars["inner_top_square_width_label"] = ttk.Label(row, text="Inner Top SqW(mm)")
        self.vars["inner_top_square_width_label"].grid(row=8, column=0, sticky="w", padx=5, pady=5)
        self.vars["inner_top_square_width_entry"] = ttk.Entry(row, textvariable=self.vars['inner_top_square_width_var'], state="readonly")
        self.vars["inner_top_square_width_entry"].grid(row=8, column=1, sticky="ew", padx=5, pady=5)

        self.vars["inner_top_square_height_label"] = ttk.Label(row, text="Inner Top SqH(mm)")
        self.vars["inner_top_square_height_label"].grid(row=8, column=2, sticky="w", padx=5, pady=5)
        self.vars["inner_top_square_height_entry"] = ttk.Entry(row, textvariable=self.vars['inner_top_square_height_var'], state="readonly")
        self.vars["inner_top_square_height_entry"].grid(row=8, column=3, sticky="ew", padx=5, pady=5)

        # --- Pieces Needed ---
        ttk.Label(row, text="Middle Dado Strips").grid(row=9, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(row, textvariable=self.vars['middle_dado_strips_var'], state="readonly").grid(row=9, column=1, columnspan=3, sticky="ew", padx=5, pady=5)

        self.vars["outer_square_strips_label"] = ttk.Label(row, text="Outer Square Strips")
        self.vars["outer_square_strips_label"].grid(row=10, column=0, sticky="w", padx=5, pady=5)
        self.vars["outer_square_strips_entry"] = ttk.Entry(row, textvariable=self.vars['outer_square_strips_var'], state="readonly")
        self.vars["outer_square_strips_entry"].grid(row=10, column=1, columnspan=3, sticky="ew", padx=5, pady=5)

        self.vars["inner_square_strips_label"] = ttk.Label(row, text="Inner Square Strips")
        self.vars["inner_square_strips_label"].grid(row=11, column=0, sticky="w", padx=5, pady=5)
        self.vars["inner_square_strips_entry"] = ttk.Entry(row, textvariable=self.vars['inner_square_strips_var'], state="readonly")
        self.vars["inner_square_strips_entry"].grid(row=11, column=1, columnspan=3, sticky="ew", padx=5, pady=5)
        ...
        self.vars['stair_tread_rise_var'].trace_add("write", trigger_on_change)
        self.vars['stair_tread_depth_var'].trace_add("write", trigger_on_change)
        self.vars['stair_step_count_var'].trace_add("write", trigger_on_change)
